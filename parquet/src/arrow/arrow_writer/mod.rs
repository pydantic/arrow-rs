...
mod tests {
    ...
    fn test_arrow_writer_caps_page_size_for_large_strings() {
        // End-to-end coverage for `ByteArrayEncoder::estimated_value_bytes_gather`
        ...
        let strings: Vec<String> = (0..num_rows).map(|_| "x".repeat(value_size)).collect();
        let batch = RecordBatch::try_new(
            schema.clone(),
        ...
        assert!(
            ...
            "expected at least {} data pages for {num_rows} large strings via ArrowWriter, got {} ({data_pages:?})",
            num_rows / 2,
            data_pages.len(),
        );
    ...
    fn test_arrow_writer_caps_page_size_for_large_string_view() {
        ...
        assert!(
            data_pages.len() >= num_rows / 2,
            "expected pages bounded by byte budget for Utf8View, got {data_pages:?}"
        );
        for size in &data_pages {
            assert!(
                *size <= value_size + 64,
