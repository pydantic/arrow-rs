...
use crate::variant_array::{BorrowedShreddingState, binary_array_value};
...
fn shredded_get_path(
    ...
    let shred_basic_variant =
        |target: VariantArray, path: VariantPath<'_>, as_field: Option<&Field>| {
            ...
            if has_path
                && target.typed_value_field().is_none()
                && let Some(value_column) = target.value_field()
            {
                let metadata_column = target.metadata_field();
